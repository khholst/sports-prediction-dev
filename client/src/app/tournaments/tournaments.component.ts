...
import { NewSpecialResultComponent } from './new-special-result/new-special-result.component';
...
export class TournamentsComponent implements OnInit {
  ...
  public specials: {[key:number]:Special[]} = {};
  ...
  async onTournamentsRequest() {
    ...
    for (let i = 0; i < this.tournaments.length; i++) {
      ...
      this.tournaments[i].start_date_pretty = this.formatService.dateStringToText(this.tournaments[i].start_date, false);
      this.tournaments[i].end_date_pretty = this.formatService.dateStringToText(this.tournaments[i].end_date, false);
      index++;
    };
  ...
  async onGamesRequest(tournID:string, isCol:boolean, index:number) {
    ...
    if(!isCol){
      show.textContent = " Hide predictions ";
      ...
      if(!(index in this.games)){
        ...
        const [specialsResponse, games] = await Promise.all([
          this.dataService.getSpecials(tournID),
          this.dataService.getGames(tournID)
        ])
        this.games[index]     = games;
        this.specials[index]  = specialsResponse.specials;

        for (let i = 0; i < this.games[index].length; i++) {
          this.games[index][i].time = this.formatService.dateStringToText(this.games[index][i].time, true);
      ...
      arrow[0].classList.toggle("down");
      arrow[1].classList.toggle("down");
      show.textContent = " Show predictions ";
    };
  };
  ...
  onNewGame(tournament: Tournament, index: number) {
    ...
    // Reflect new special in opened game list
    modal.componentInstance.newSpecial.subscribe((newSpecial: Special) => {
      if (Object.keys(this.specials).length > 0) {
        this.specials[index].push(newSpecial);
      }
    });
  }

  onSpecialResult(special: Special, index: number) {
    const modal = this.modalService.open(NewSpecialResultComponent);
    modal.componentInstance.specialPrediction = special;
